...
def get_properties(property_name: str, property_output_file: str, input_id_file: str) -> pandas.DataFrame:
    ...
    for material in materials.splitlines():
        ...
        with open(f'{material_name}-geometry.json', 'r') as j:
            contents = json.loads(j.read())
        df = pandas.json_normalize(contents['geometricProperties'])
        df = df[df['name'] == property_name]
        concat_result = pandas.DataFrame([[material, df['value'].values[0]]],
                                         columns=['input-id', str(df['name'].values[0])])
